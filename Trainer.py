...
class BayesianTrainer(object):
    def __init__(self, model, criterion, lr_scheduler, logger, train_set_size, log_params=True):
        self.model = model
        ...
        if(lr_scheduler == 'cosine'):
            self.lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optim, T_max=500, eta_min=1e-16, last_epoch=-1)
        else:
    ...
    def train(self, epochs, trainloader, device='cpu'):
        for epoch in range(epochs):
            ...
            self.logger.info("train correct: {}".format(sum(predictions) / len(predictions)))
