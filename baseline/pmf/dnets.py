...
def get_arguments():
    ...
    parser.add_argument(
        "--input-channels", type=int, default=INPUT_CHANNELS, help="Input channels."
    )
    parser.add_argument("--im-size", type=int, default=IM_SIZE, help="Image size.")
    parser.add_argument(
        "--input-dim", type=int, default=INPUT_DIM, help="Input dimension."
    )
    parser.add_argument(
        "--hidden-dim", type=int, default=HIDDEN_DIM, help="Hidden layer size."
    )
    parser.add_argument(
        "--output-dim", type=int, default=OUTPUT_DIM, help="Output dimension."
    )
    parser.add_argument(
        "--dataset-size", type=int, default=DATASET_SIZE, help="No of datapoints."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num-iters", type=int, default=NUM_ITERS, help="Number of iterations."
    )
    parser.add_argument(
        "--num-epochs", type=int, default=NUM_EPOCHS, help="Number of epochs."
    )
    parser.add_argument(
        "--save-dir", type=str, default=SAVE_DIR, help="Output directory."
    )
    parser.add_argument(
        "--gpu-id", type=str, default=GPU_ID, help="Cuda visible device."
    )
    parser.add_argument(
        "--optimizer",
        type=str,
    ...
    parser.add_argument(
        "--rounding", type=str, default=ROUNDING, help="Type of rounding [ARGMAX]."
    )
    parser.add_argument(
        "--eval-set",
        type=str,
        default=EVAL_SET,
        help="Dataset to evaluate [TEST, TRAIN].",
    )
    parser.add_argument(
        "--val-set",
        type=str,
        default=VAL_SET,
        help="Dataset to validate [TEST, TRAIN].",
    )
    parser.add_argument(
        "--loss-function",
        type=str,
    ...
    parser.add_argument(
        "--nesterov", action="store_true", help="Flag to use Nesterov momentum."
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    parser.add_argument(
        "--beta-scale", type=float, default=BETA_SCALE, help="Scale to multiply beta."
    )
    parser.add_argument(
        "--lr-scale",
        type=float,
        default=LR_SCALE,
        help="Scale to multiply learning rate.",
    )
    parser.add_argument(
        "--lr-interval",
        type=str,
    ...
    parser.add_argument(
        "--save-name", type=str, default=SAVE_NAME, help="Name to save the best model."
    )
    parser.add_argument(
        "--pretrained-model",
        type=str,
        default=PRETRAINED_MODEL,
        help="Pretrained model to load weights.",
    )
    parser.add_argument(
        "--data-path", type=str, default=DATA_PATH, help="Path to store datasets."
    )
    parser.add_argument(
        "--eval", type=str, default=EVAL, help="Model file to evaluate."
    )
    parser.add_argument(
        "--resume", type=str, default=RESUME, help="Checkpoint file to resume training."
    )
...
def main():
    ...
    if args.num_epochs == 0:
        args.num_epochs = int(
            np.ceil(float(args.num_iters) * args.batch_size / float(args.dataset_size))
        )
    ...
    test_loader = du.DataLoader(
        test_set, batch_size=args.batch_size, worker_init_fn=init_fn, **kwargs
    )
